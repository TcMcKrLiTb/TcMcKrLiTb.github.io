...
class WopiController extends Controller {
	...
	public function CheckFileInfo($fileId, $access_token): JSONResponse {
		...
		if (isset($tokenData['UserId'])) {
			$file = $this->getFileById($tokenData['UserId'], $fileId);
			...
			$folderShareUrl = $this->urlGenerator->linkToRouteAbsolute('files.view.index') . "?dir=$locationInUrl&fileid=$fileId&details=shareTabView";
			$hostEditUrl = $this->urlGenerator->linkToRouteAbsolute('wopi.page.Office', ['_action' => 'edit', 'fileId' => $fileId]);
			$hostViewUrl = $this->urlGenerator->linkToRouteAbsolute('wopi.page.Office', ['_action' => 'view', 'fileId' => $fileId]);
			...
			$file = $this->getByShareToken($tokenData['ShareToken'], $fileId);
			...
			$hostViewUrl = $this->urlGenerator->linkToRouteAbsolute('wopi.page.OfficePublicLink', ['_action' => 'view', 'shareToken' => $tokenData['ShareToken'], 'fileId' => $fileId]);
	...
	public function FileOperation(string $fileId, string $access_token) : JSONResponse {
		...
		if (isset($tokenData['UserId'])) {
			...
			$this->logger->error(
				"FileOperation {$header} for {$fileId}
		...
		switch ($header) {
			...
			case 'PUT_RELATIVE':
				...
				$suggestedTarget = \iconv('utf-7', 'utf-8',
					$this->request->getHeader('X-WOPI-SuggestedTarget'));
				$relativeTarget = \iconv('utf-7', 'utf-8',
	...
	public function GenerateNewAuthUserAccessToken($fileId, $folderUrl): JSONResponse {
		...
		return new JSONResponse(
			$this->tokenService->GenerateNewAuthUserAccessToken(
				$fileId, $folderUrl, $this->userSession->getUser()));
	}
	 ...
	 * @throws NotFoundException
	 */
	private function putFileRelative(File $sourceFile,
									 $suggestedTarget, $relativeTarget,
									 $overwrite, $fileConversion) : JSONResponse {
	 ...
	 * @throws NotPermittedException
	 */
	private function getFileById(string $userId, string $fileId): File {
		// setupFS and setUser is required to have a proper activity entry
		OC_Util::setupFS($userId);
		$this->userSession->setUser(
			$user = $this->userManager->get($userId));

		$file = $this->fileService->getByFileId(\intval($fileId));

		if ($file instanceof File) {
			return $file;
		}
		throw new NotPermittedException("Resource with $fileId is not a file");
	}
	 ...
	 * @throws HintException
	 */
	private function getByShareToken($shareToken, $fileId): File {
		$file = $this->fileService->getByShareToken($shareToken, \intval($fileId));
		if ($file instanceof File) {
			return $file;
		}
		throw new NotPermittedException("Resource with $fileId is not a file");
	 ...
	 * @throws NotFoundException
	 */
	private function lock(IPersistentLockingStorage $storage,
						  File $file, string $wopiLock, array $tokenData): JSONResponse {
	 ...
	 * @return JSONResponse
	 */
	private function refreshLock(IPersistentLockingStorage $storage,
								 File $file,
								 string $wopiLock): JSONResponse {
		$lock = $this->getLockByToken($storage, $file, $wopiLock);
		if ($lock === null) {
			$response = new JSONResponse([], Http::STATUS_CONFLICT);
	 ...
	 * @throws NotFoundException
	 */
	private function unlock(IPersistentLockingStorage $storage, File $file,
							string $wopiLock): JSONResponse {
		$lock = $this->getLockByToken($storage, $file, $wopiLock);
		if ($lock === null) {
			$response = new JSONResponse([], Http::STATUS_CONFLICT);
	 ...
	 * @return null|ILock
	 */
	private function getLockByToken(IPersistentLockingStorage $storage,
									FileInfo $file, string $wopiLock) : ?ILock {
		$locks = $storage->getLocks($file->getInternalPath(), false);
		if (empty($locks)) {
			return null;
		}
	 ...
	 */
	private function verifyLock(IPersistentLockingStorage $storage,
								FileInfo $file,
								 ?string $wopiLock): ?JSONResponse {
		$lock = $this->getLockByToken($storage, $file, $wopiLock ?? '');
	...
	private function getWopiSrc(File $targetFile, IUser $user) : string {
		$newToken = $this->tokenService->GenerateNewAuthUserAccessToken(
			(string) $targetFile->getId(), $targetFile->getParent()->getPath(), $user
		);
		$queryParameters = [
